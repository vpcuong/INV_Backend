...
const VALID_PO_LINE_TRANSITIONS: Record<POLineStatus, POLineStatus[]> = {
  [POLineStatus.OPEN]: [
    POLineStatus.PARTIALLY_RECEIVED,
...
export class PoService {
  ...
  async create(createDto: CreatePOHeaderDto, createdBy: string) {
    ...
    return this.repository.create({
      ...
      note: createDto.note,
      createdBy,
      lines: createDto.lines?.map(
        (line): CreatePOLineData => ({
  ...
  async updateWithLines(
    ...
    if (dto.header?.status) {
      this.validatePOStatusTransition(
        poHeader.status,
        dto.header.status as POStatus
      );
    }
    ...
    if (dto.lines) {
      for (const line of dto.lines) {
        if (line.id && line.status) {
          ...
          if (existingLine) {
            this.validatePOLineStatusTransition(
              existingLine.status,
              line.status as POLineStatus
            );
          }
        }
      }
  ...
  async updateLineReceivedQty(
    ...
    if (!line) {
      throw new NotFoundException(
        `PO line with skuId ${skuId} not found in PO ${poId}`
      );
    }
    ...
    await this.repository.updateLineReceivedQty(
      line.id,
      newReceivedQty,
      newLineStatus
    );
  ...
  async recalculatePOStatus(poId: number) {
    ...
    const allReceived = po.lines.every(
      (l) => l.status === POLineStatus.RECEIVED
    );
    const someReceived = po.lines.some(
      (l) =>
        l.status === POLineStatus.RECEIVED ||
  ...
  private validatePOLineStatusTransition(
    current: POLineStatus,
    next: POLineStatus
  ) {
