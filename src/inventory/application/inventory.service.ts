...
export class InventoryService {
  ...
  async create(dto: CreateInvTransHeaderDto, createdBy: string): Promise<any> {
    ...
    const transNum = await this.numberGenerator.generate(
      dto.type as InvTransType
    );
    ...
    for (const lineDto of dto.lines) {
      ...
      if (!item) {
        throw new BadRequestException(
          `Item SKU ${lineDto.itemSkuId} not found`
        );
      }
      ...
      if (sku.getUomCode() !== lineDto.uomCode) {
        ...
        // Convert quantity
        const convertedQty = uomClass.convertQuantity(
          lineDto.uomCode,
          sku.getUomCode(),
          lineDto.quantity
        );
        // Find conversion
        const uomConv = uomClass
          .getConversions()
          .find((u) => u.getUomCode() === sku.getUomCode());
        // Resolve
  ...
  async createGoodsReceiptFromPo(
    dto: CreateGoodsReceiptFromPoDto,
    createdBy: string
  ): Promise<any> {
    // Validate Warehouse
    await this.validateWarehouse(dto.toWarehouseId);
  ...
  async createGoodsIssueFromSo(
    dto: CreateGoodsIssueFromSoDto,
    createdBy: string
  ): Promise<any> {
    // Validate Warehouse
    await this.validateWarehouse(dto.fromWarehouseId);
    ...
    const pendingTransactions = await this.repository.findByReference(
      'SO',
      so.getId()
    );
    const pendingDrafts = pendingTransactions.filter(
      (t) =>
        t.getType() === InvTransType.GOODS_ISSUE &&
        t.getStatus() === InvTransStatus.DRAFT
    );
    ...
    for (const lineDto of dto.lines) {
      ...
      const soLine = so
        ...
        .find(
          (l) =>
            l.getItemSkuId() === sku.getId() &&
            (l.getStatus() === 'OPEN' || l.getStatus() === 'PARTIAL')
        );

      if (!soLine) {
        throw new BadRequestException(
          `Item SKU ${lineDto.itemSkuId} not found in open SO lines`
        );
      }
      // Calculate pending qty for this SKU
      const pendingQty = pendingDrafts.reduce((sum, t) => {
        const matchingLines = t
          .getLines()
          .filter((l) => l.getItemSkuId() === sku.getId());
      ...
      */
      ...
      const totalRequested =
        Number(lineDto.quantity) +
        Number(pendingQty) +
        Number(soLine.getShippedQty()); // Simplistic accumulation
  ...
  async findByPublicId(publicId: string): Promise<any> {
    ...
    if (!header) {
      throw new NotFoundException(
        `Transaction with Public ID ${publicId} not found`
      );
    }
    return this.toDto(header);
  }
  ...
  async completeByPublicId(publicId: string): Promise<any> {
    ...
    if (
      header.getType() === InvTransType.GOODS_RECEIPT &&
      header.getReferenceType() === 'PO' &&
    ...
    if (
      header.getType() === InvTransType.GOODS_ISSUE &&
      header.getReferenceType() === 'SO' &&
      header.getReferenceId()
    ) {
      try {
