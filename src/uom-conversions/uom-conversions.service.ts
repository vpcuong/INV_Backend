...
export class UomConversionsService {
  ...
  async create(createUomConversionDto: CreateUomConversionDto) {
    ...
    if (!classUOMCode) {
      throw new NotFoundException(
        `From UOM Class with Code ${createUomConversionDto.uomClassCode} not found`
      );
    }

    if (!uomCode) {
      throw new NotFoundException(
        `To UOM with Code ${createUomConversionDto.uomCode} not found`
      );
    }
  ...
  async findOne(uomClassCode: string, uomCodeString: string) {
    ...
    if (!conversion) {
      throw new NotFoundException(
        `UOM conversion for class '${uomClassCode}' and UOM '${uomCodeString}' not found`
      );
    }
  ...
  async update(
    uomClassCode: string,
    uomCode: string,
    updateUomConversionDto: UpdateUomConversionDto
  ) {
    ...
    if (updateUomConversionDto.uomCode) {
      ...
      if (!uomCode) {
        throw new NotFoundException(
          `UOM with code: ${updateUomConversionDto.uomCode} not found`
        );
      }
    }
  ...
  async remove(uomClassCode: string, uomCode: string) {
    ...
    if (!conversion) {
      throw new NotFoundException(
        `UOM conversion with ID ${uomClassCode} not found`
      );
    }
